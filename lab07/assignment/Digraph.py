...
def sap(g, aList, bList):
    res=None    
    queue=Queue()
    visitedA = [False for _ in range(g.V)]
    visitedB = [False for _ in range(g.V)]
    distanceA = [ 0 for _ in range(g.V)]
    distanceB = [0 for _ in range(g.V)]
    for v in aList:
        queue.put((v,'a',0))
        if v in bList:
            return (v,0)
        ...
        for w in g.adj[v[0]]:
            if v[1]=='a':
                ...
                if not visitedA[w]:
                    visitedA[w]=True
                    distanceA[w]=v[2]+1
                    queue.put((w,'a',distanceA[w]))
                if visitedB[w]:
                    dis=distanceA[w]+distanceB[w]
                    if dis<sapLangth:
                        sapLangth=dis
                        scp=w

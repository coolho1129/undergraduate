...
class SeamCarver:
    ...
    def findVerticalSeam(self):
        ...
        h = self.height()
        w = self.width()
        distTo = [[self.MAX_ENERGY if y==0 else 0 for _ in range(w)] for y in range(h)]
        edgeTo = [[None if y==0 else 0 for _ in range(w)] for y in range(h)]
        
        for y in range(1,h):
            for x in range(0,w):
                if x == 0:
                    mindist = min(distTo[y-1][x],distTo[y-1][x+1])
                elif x + 1 == w:
                    mindist = min(distTo[y-1][x],distTo[y-1][x-1])
                else:
                    mindist = min(distTo[y-1][x],distTo[y-1][x-1],distTo[y-1][x+1])
                    
                if mindist == distTo[y-1][x]:
                    min_idx = x
                elif mindist == distTo[y-1][x-1]:
                    min_idx = x-1
                else:
                    min_idx = x+1
                    
                distTo[y][x] = mindist + self.energy(x,y)
                edgeTo[y][x] = min_idx

        res = [0] * h
        x_min = edgeTo[h-1][distTo[h-1].index(min(distTo[h-1]))]

...
class FlowEdge:
    ...
    def addRemainingFlowTo(self, vertex, delta): # Add delta flow toward vertex     
        ...
        assert delta <= self.remainingCapacityTo(vertex), f"Delta {delta} is greater than the remaining capacity {self.remainingCapacityTo(vertex)}"        
...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        res=[]
        otherid = []
        maxwin = self.wins[teamid] + self.remaining[teamid]
        ...
        otherVcnt = N - 1
        startendVcnt = 2
        vcnt = startendVcnt + otherVcnt + gameVcnt
        g = FlowNetwork(vcnt)
        start = 0
        end = vcnt-1
        games = []
        ...
        for idx, i in enumerate(otherid):
            id2v[i]= gameVcnt + idx + 1
            for j in otherid[idx+1:]:
                games.append((i,j,gameV))
                gameV+=1   
                
        # 시작점 -> 게임정점       
        for i,game in enumerate(games):
            g.addEdge(FlowEdge(start,game[2], self.against[game[0]][game[1]]))
        
        # 게임정점 -> 팀 정점
        for i,game in enumerate(games):
            g.addEdge(FlowEdge(game[2],id2v[game[0]],float('inf')))
            g.addEdge(FlowEdge(game[2],id2v[game[1]],float('inf')))
        
        # 팀 정점 -> 종료점
        for id in otherid:
            g.addEdge(FlowEdge(id2v[id], end,  maxwin - self.wins[id]))
        
        ffbase = FordFulkerson(g,start,end) 
      
        for id in otherid:
            if ffbase.inCut(id2v[id]):
                res.append(self.teams[id])
       
        return (True, res) if len(res) > 0 else(False, res)

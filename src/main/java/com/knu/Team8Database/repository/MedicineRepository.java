...
public interface MedicineRepository extends JpaRepository<Detail_view, String> {
    ...
    @Query("SELECT DISTINCT D.medicineId AS medicineId, D.medicineName AS medicineName, D.componentName AS componentName, D.symtomName AS symtomName, D.companyName AS companyName " +
            "FROM Detail_view D WHERE (D.medicineName LIKE %:medicineName%) AND (D.symtomName LIKE %:symtomName%)" +
            " AND (D.companyName LIKE %:companyName%) AND (D.componentName LIKE %:componentName%)" +
            " AND (D.medicinePrice >= :medicinePrice) AND (D.symtomField LIKE %:symtomField%)")
    List<Detail_viewDTO> find_simple(@Param("medicineName") String medicineName, @Param("componentName") String componentName,

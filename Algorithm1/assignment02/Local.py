...
def main():
    ...
    for row in range(2,len(x)+2):
        for col in range(2,len(y)+2):
            ...
            score_matrix[row][col]=max(0,score_matrix[row-1][col-1]+s,score_matrix[row][col-1]+gap,score_matrix[row-1][col]+gap)
            
            if(score_matrix[row][col]==0):
                backtrack_matrix[row][col]='_'
            
            elif(score_matrix[row][col]==score_matrix[row-1][col-1]+s):
                 backtrack_matrix[row][col]='↖'
              
            
            elif(score_matrix[row][col]==score_matrix[row][col-1]+gap):
                backtrack_matrix[row][col]='←'
            
          
            elif(score_matrix[row][col]==score_matrix[row-1][col]+gap):
                backtrack_matrix[row][col]='↑'
